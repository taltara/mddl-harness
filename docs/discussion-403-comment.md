Corroborating this from the other direction, in case it helps confirm the diagnosis: I ship a tool that writes custom presets, and it works — precisely because it writes to the default user root and never touches `roots`.

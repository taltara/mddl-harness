...
I had missed that `--dump-default-config` deliberately skips the user layers, so I confirmed it on rc.7 before repeating it: writing a uniquely-named row into a profile's `cordis.patch.yml`, `--dump-config` shows it and `--dump-default-config` does not. That is exactly the probe you want when the thing you would normally debug with is the thing that is gone, and it deserves to be in the config docs rather than discovered during an incident.
